...
describe('Favorites production panel', () => {
  ...
  it('offers sidebar import only until a snapshot exists and leaves refresh and clear to Settings', async () => {
    const f = fixture(); const importHistory = vi.fn(), clearHistory = vi.fn()
    f.renderer.dispose()
    const renderer = new FavoritesPanelRenderer(f.container, { ...f.actions, importHistory, clearHistory })
    cleanups.push(() => renderer.dispose())
    const state = applyFavoritesOperation(f.state, { type: 'favorites:preferences', patch: { activeTab: 'recent' } }, 'darwin')
    const initial = { state, current: f.current, platform: 'darwin' as const, writable: true, history: normalizeHistory(undefined, 'darwin'), historyImport: { available: true, loading: false } }
    renderer.update(initial)
    expect(f.container.querySelector('[data-recent-order]')?.textContent).toBe('Not imported')
    expect(f.container.textContent).toContain('Settings')
    f.button('Import from Typora').click(); await f.flush(); expect(importHistory).toHaveBeenCalledOnce()
    const history = normalizeHistory({ status: 'ready', order: 'global', entries: [{ kind: 'file', path: '/Fixture/Recent.md' }] }, 'darwin')
    renderer.update({ ...initial, history, historyImport: { available: true, loading: false, importedAt: 1000 } })
    expect(f.container.querySelector('[data-key="history-import"]')).toBeNull()
    expect(f.container.querySelector('[data-key="history-clear"]')).toBeNull()
    const order = f.container.querySelector<HTMLElement>('[data-recent-order]')!
    expect(order.textContent).toBe('Most recent first')
    expect(order.title).toContain('not live'); expect(order.title).toContain('Settings')
    expect(clearHistory).not.toHaveBeenCalled()
  })
  it('calls only a dated snapshot most recent first and explains how to enable Recently opened', () => {
    const f = fixture()
    const state = applyFavoritesOperation(f.state, { type: 'favorites:preferences', patch: { activeTab: 'recent' } }, 'darwin')
    const perKind = normalizeHistory({ status: 'ready', order: 'per-kind', entries: [{ kind: 'file', path: '/Fixture/Recent.md' }] }, 'darwin')
    f.renderer.update({ state, current: f.current, platform: 'darwin', writable: true, history: perKind, historyImport: { available: true, loading: false, importedAt: 1000 } })
    expect(f.container.querySelector('[data-recent-order]')?.textContent).toBe('Typora\'s order')
    f.renderer.update({ state: f.state, current: f.current, platform: 'darwin', writable: true, history: normalizeHistory(undefined, 'darwin') })
    f.button('Items: A-Z').click()
    expect(f.container.querySelector('.qa-sort-options')?.textContent).toContain('Import Typora Recent')
  })
  ...
  it('uses frozen defaults, header controls, mixed groups, and no whole Favorites collapse in tabs', () => {
    ...
    expect(f.button('View')).toBeTruthy(); expect(f.button('Groups: Custom')).toBeTruthy(); expect(f.button('Items: A-Z')).toBeTruthy()
  ...
  it('applies full-label View choices and independent layout/group view preferences', async () => {
    const f = fixture(); f.button('View').click()
  ...
  it('closes popovers on Escape and restores the trigger focus', () => {
    ...
    expect(document.activeElement).toBe(f.button('View'))
  })
  ...
  it('outside dismissal leaves the pointer target connected so its click still executes', () => {
    const f = fixture(); f.button('View').click(); const manage = f.button('Manage groups')
